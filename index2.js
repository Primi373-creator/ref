...
const botToken = "7020781359:AAEi5tH3lOmM44tFaz7ZCiDOGxrd2jCQKq0";
const bot = new TelegramBot(botToken, { polling: true });
const adminId = "6551258524";

async function isUserSubscribed(username) {
  try {
    const chatMember = await bot.getChatMember("@hackersssd", username);

    const isSubscribed =
      chatMember.status === "member" || chatMember.status === "administrator";

    console.log(
      `User ${username} is${isSubscribed ? "" : " not"} subscribed to the channel.`,
    );
...
bot.onText(/\/start/, async (msg) => {
  ...
  if (isSubscribed) {
    try {
      ...
      bot.sendMessage(
        chatId,
        `Welcome to the referral bot! ${isAdmin ? "You are an admin." : ""} Use the buttons below:`,
        {
...
bot.on("callback_query", (query) => {
  ...
  isUserSubscribed(userId).then((isSubscribed) => {
    if (isSubscribed) {
      switch (data) {
        ...
        case "subscribe_to_channel":
          ...
          bot.answerCallbackQuery(query.id, {
            text: "Click the button below to subscribe to our channel.",
          });
          bot.sendInlineKeyboard(userId, [
            [{ text: "Subscribe", url: channelLink }],
          ]);
          break;
        ...
        case "set_referral_requirement":
          if (userId.toString() === adminId) {
            bot.sendMessage(
              userId,
              "Please use the /setreferralcount command to set the referral requirement.",
            );
          } else {
            bot.answerCallbackQuery(query.id, {
        ...
        case "set_text_file":
          if (userId.toString() === adminId) {
            bot.sendMessage(
              userId,
              "Please use the /setfile command to set a text file.",
            );
          } else {
            bot.answerCallbackQuery(query.id, {
              text: "You are not authorized to use this command.",
        ...
        case "get_file":
          User.findOne({ telegramId: userId }, (err, user) => {
            ...
            if (user.referrals >= bot.referralRequirement) {
              bot.sendMessage(
                userId,
                "Please enter the ID of the file you want to receive.",
              );
              bot.once("text", (msg) => {
                ...
                if (fileText) {
                  bot.sendMessage(
                    userId,
                    `Here is your file with ID ${fileId}:\n${fileText}`,
                  );
                } else {
              ...
              bot.answerCallbackQuery(query.id, {
                text: `You need ${bot.referralRequirement - user.referrals} more referrals to get a file.`,
              });
            }
          });
          break;
        ...
        case "set_admin":
          User.findOne({ telegramId: userId }, (err, user) => {
            ...
            if (user.isAdmin) {
              bot.sendMessage(
                userId,
                "Please enter the Telegram ID of the user you want to set as an admin.",
              );
              bot.once("text", (msg) => {
                ...
                User.findOneAndUpdate(
                  ...
                  (err, updatedAdminUser) => {
                    if (err || !updatedAdminUser) {
                      bot.sendMessage(
                        userId,
                        "Error setting admin. Please try again.",
                      );
                    } else {
                      bot.sendMessage(
                        userId,
                        `User with ID ${adminUserId} is now an admin.`,
                      );
                    }
                  },
                );
        ...
        case "add_channel":
          User.findOne({ telegramId: userId }, (err, user) => {
            ...
            if (user.isAdmin) {
              bot.sendMessage(
                userId,
                "Please enter the link of the channel you want to add for users to subscribe to.",
              );
              bot.once("text", (msg) => {
                const channelLink = msg.text.trim();
                bot.sendMessage(
                  userId,
                  `Channel ${channelLink} has been added.`,
                );
              });
            } else {
              bot.answerCallbackQuery(query.id, {
      ...
      bot.answerCallbackQuery(query.id, {
        text: "To use this command, please subscribe to our channel first.",
      });
    }
  });
});
bot.onText(/\/setreferralcount/, (msg) => {
  ...
  User.findOne({ telegramId: userId }, (err, user) => {
    ...
    if (user.isAdmin) {
      bot.sendMessage(
        chatId,
        "Please enter the new referral requirement count.",
      );
      bot.once("text", (msg) => {
        const newRequirement = parseInt(msg.text.trim());
        bot.sendMessage(
          chatId,
          `Referral requirement set to ${newRequirement}.`,
        );
      });
    } else {
...
bot.onText(/\/setfile/, (msg) => {
  ...
  User.findOne({ telegramId: userId }, (err, user) => {
    ...
    if (user.isAdmin) {
      bot.sendMessage(
        chatId,
        'Please enter the ID and text of the file in the format "ID:Text".',
      );
      bot.once("text", (msg) => {
...
bot.onText(/\/addadmin/, (msg) => {
  ...
  User.findOne({ telegramId: userId }, (err, user) => {
    ...
    if (user.isAdmin) {
      bot.sendMessage(
        chatId,
        "Please enter the Telegram ID of the user you want to add as an admin.",
      );
      bot.once("text", (msg) => {
        const adminUserId = parseInt(msg.text.trim());
        User.findOneAndUpdate(
...
bot.onText(/\/setcontacttext/, (msg) => {
  ...
  User.findOne({ telegramId: userId, isAdmin: true }, (err, user) => {
    ...
    bot.sendMessage(
      chatId,
      "Please enter the new text for the /contact command.",
    );
    bot.once("text", (msg) => {
...
bot.onText(/\/setchanneltext/, (msg) => {
  ...
  User.findOne({ telegramId: userId, isAdmin: true }, (err, user) => {
    ...
    bot.sendMessage(
      chatId,
      "Please enter the new text for the /viewchannel command.",
    );
    bot.once("text", (msg) => {
...
bot.onText(/\/viewchannel/, (msg) => {
  ...
  User.findOne({ telegramId: userId }, (err, user) => {
    ...
    const channelText =
      "To view the channel, please subscribe to our channel first.";
